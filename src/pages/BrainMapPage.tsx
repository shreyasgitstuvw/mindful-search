...
export default function BrainMapPage() {
  ...
  const points = useMemo(() => (content ? projectTo2D(content) : []), [content]);
  ...
  return (
    <div className="container py-8 space-y-6">
      ...
      <div className="rounded-lg border border-border bg-card p-4">
        <ResponsiveContainer width="100%" height={500}>
          <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            <XAxis type="number" dataKey="x" name="Stimulation" tick={{ fontSize: 10, fill: 'hsl(215, 15%, 55%)' }} axisLine={{ stroke: 'hsl(220, 20%, 18%)' }} tickLine={false} label={{ value: 'Visual + Reward Axis', position: 'bottom', fontSize: 10, fill: 'hsl(215, 15%, 55%)' }} />
            <YAxis type="number" dataKey="y" name="Emotion" tick={{ fontSize: 10, fill: 'hsl(215, 15%, 55%)' }} axisLine={{ stroke: 'hsl(220, 20%, 18%)' }} tickLine={false} label={{ value: 'Emotional Axis', angle: -90, position: 'left', fontSize: 10, fill: 'hsl(215, 15%, 55%)' }} />
            <Tooltip content={<CustomTooltip />} />
            <Scatter data={points}>
              {points.map((entry) => (
                <Cell key={entry.id} fill={tribeHexMap[entry.tribe] || '#1D9E75'} fillOpacity={hoveredId === entry.id ? 1 : 0.7} r={hoveredId === entry.id ? 8 : 5} onMouseEnter={() => setHoveredId(entry.id)} onMouseLeave={() => setHoveredId(null)} cursor="pointer" />
              ))}
            </Scatter>
          </ScatterChart>
        </ResponsiveContainer>
      ...
      <div className="flex flex-wrap gap-3">
        {Object.entries(tribeHexMap).map(([tribe, color]) => (
